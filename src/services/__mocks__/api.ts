...
export async function getSideDishList(): Promise<SideDishMedia[]> {
  return [{ id: 1, media_type: "audio", side_dish: "Mock Side Dish" }];
}

export async function getDrinkStyleList(): Promise<DrinkStyle[]> {
  return [{ id: 1, style: "Mock Style", drink: "Mock Drink" }];
}

// printjob.py
export async function createPrintJob(
  data: Omit<PrintJob, "id">,
): Promise<PrintJob> {
  return {
    id: 1,
    ...data,
...
// textvariant.py
export async function pickBestTextVariant(params: {
...
export async function createBarcodeMapping(
  data: BarcodeMappingCreate,
...
export async function deleteBarcodeMapping(
  _item_id: number,
): Promise<{ ok: boolean }> {
  return { ok: true };
}
